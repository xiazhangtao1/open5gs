...
## VPP单Worker、UPF五Worker的单/五Session双向性能（2026-08-09）

Helm revision 114实际部署VPP 2个逻辑CPU和1个VPP Worker，`vpp_main`运行于
CPU 50，`vpp_wk_0`运行于CPU 122；两者是同一物理核的两个超线程，调度策略
均为`SCHED_OTHER`。UPF固定5个Session Worker，N3/N6 memif各建立5组ring。

测试报文为1428-byte inner IPv4 UDP，采用10微秒均匀节奏，每档持续20秒。
扫描总速率为1/2/4/6/8/10Gbps。下行计数口径为VPP发送到N6 memif的包数与
VPP从N3 memif收到的包数；上行计数口径为VPP发送到N3 memif的包数与VPP从
N6 memif收到的包数。两种方向均经过Open5GS完整UPF语义处理。

单Session场景只向`10.45.0.2`灌流，该Session由一个owner Worker处理，其余
Worker保持空闲：

| 目标速率 | 输入包数 | 下行输出包数 | 下行丢包率 | 上行输出包数 | 上行丢包率 |
|---:|---:|---:|---:|---:|---:|
| 1 Gbps | 1,750,700 | 1,750,700 | 0% | 1,750,700 | 0% |
| 2 Gbps | 3,501,400 | 3,501,400 | 0% | 3,501,400 | 0% |
| 4 Gbps | 7,002,800 | 7,002,800 | 0% | 7,002,800 | 0% |
| 6 Gbps | 10,504,200 | 10,504,200 | 0% | 10,504,200 | 0% |
| 8 Gbps | 14,005,600 | 14,005,600 | 0% | 14,005,600 | 0% |
| 10 Gbps | 17,507,000 | 17,507,000 | 0% | 17,507,000 | 0% |

五Session场景向`10.45.0.2`至`10.45.0.6`等分总流量；例如总计10Gbps时，
每个Session目标为2Gbps。UE实际还建立了第六个PFCP Session，但该Session
全程无测试流量，不计入五Session场景：

| 总目标速率 | 每Session目标 | 输入包数 | 下行输出包数 | 下行丢包率 | 上行输出包数 | 上行丢包率 |
|---:|---:|---:|---:|---:|---:|---:|
| 1 Gbps | 0.2 Gbps | 1,750,700 | 1,750,700 | 0% | 1,750,700 | 0% |
| 2 Gbps | 0.4 Gbps | 3,501,400 | 3,501,400 | 0% | 3,501,400 | 0% |
| 4 Gbps | 0.8 Gbps | 7,002,800 | 7,002,800 | 0% | 7,002,800 | 0% |
| 6 Gbps | 1.2 Gbps | 10,504,200 | 10,504,200 | 0% | 10,504,200 | 0% |
| 8 Gbps | 1.6 Gbps | 14,005,600 | 14,005,600 | 0% | 14,005,600 | 0% |
| 10 Gbps | 2.0 Gbps | 17,507,000 | 17,507,000 | 0% | 17,507,000 | 0% |

五Session下行6Gbps首次扫描在停流读取时少4包，紧接的8Gbps扫描多计4包，
属于跨越接口计数清零边界的在途包。单独重新执行6Gbps/20秒后输入和输出均
为10,504,200包，确认核心路径丢包率为0。

测试后五个UPF Worker的`drops`、`queue-full`和`push-fail`均为0；N3/N6各
TX qid的`alloc-short`、`alloc-fail`和`drop`也全部为0。本轮证明在VPP内置
PG→memif→Open5GS完整UPF语义→memif→VPP的同机路径中，1个VPP Worker和
