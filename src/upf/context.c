...
static int upf_context_validation(void)
{
    ...
    if (upf_self()->dataplane.worker_busy_poll_us < -1 ||
        upf_self()->dataplane.worker_busy_poll_us > OGS_USEC_PER_SEC) {
        ogs_error("upf.dataplane.session_workers.busy_poll_us must be "
                "-1, 0, or between 1 and 1000000");
        return OGS_ERROR;
    }
